    ...
    <section style={{ padding:"100px 32px", background:"var(--bg)", borderTop:"1px solid var(--line)" }}>
      <div style={{ maxWidth:1160, margin:"0 auto", display:"grid", gridTemplateColumns:"1fr 1fr", gap:100, alignItems:"start" }} className="vdna-g">
        <div>
          ...
          <p style={{ fontSize:16, lineHeight:1.82, color:"var(--fg-2)", fontFamily:"'Inter', sans-serif", fontWeight:300, marginBottom:14 }}>
            A 15-minute conversation. Three extracted layers. A Voice Fidelity Score that climbs with every session. The longer you use IdeasOut, the more it sounds like you.
          </p>
        ...
        <div>
          ...
          <div style={{ padding:"28px 32px", background:"var(--surface)", border:"1px solid var(--line)", borderRadius:12, marginBottom:24, boxShadow:"var(--shadow-sm)" }}>
            ...
            <div style={{ display:"flex", alignItems:"baseline", gap:8 }}>
              <span style={{ fontFamily:"'Inter', sans-serif", fontSize:72, fontWeight:400, color:"#F5C642", letterSpacing:"-3px", lineHeight:1 }}>94.7</span>
              <span style={{ fontSize:13, color:"var(--fg-3)", fontFamily:"'Inter', sans-serif", fontWeight:300 }}>/100 · ↑ 2.3 this week</span>
            </div>
          </div>
          {/* Layers */}
          {LAYERS.map((l,i) => (
            <div key={i} style={{ marginBottom:20 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
                <span style={{ fontSize:13, fontWeight:400, color:"var(--fg)", fontFamily:"'Inter', sans-serif" }}>{l.name} Layer</span>
                <span style={{ fontSize:13, fontWeight:500, color:"#F5C642", fontFamily:"'Inter', sans-serif" }}>{l.pct}%</span>
              </div>
              ...
              <p style={{ fontSize:13, color:"var(--fg-3)", fontFamily:"'Inter', sans-serif", fontWeight:300 }}>{l.note}</p>
            </div>
          ))}
        </div>
      </div>
