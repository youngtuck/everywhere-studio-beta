    ...
    <section style={{ padding:"100px 32px", background:"var(--bg)", borderTop:"1px solid var(--line)" }}>
      <div style={{ maxWidth:1160, margin:"0 auto" }}>
        <div style={{ maxWidth:480, marginBottom:64 }}>
          ...
          <h2 style={{ fontFamily:"'Inter', sans-serif", fontSize:"clamp(32px,3.8vw,56px)", fontWeight:400, letterSpacing:"-0.5px", color:"var(--fg)", lineHeight:1.08 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8 }} className="fmt-g">
          {FORMATS.map((f,i) => (
            <div key={i}
              ...
              <p style={{ fontSize:13, fontWeight:500, color:"var(--fg)", marginBottom:4, fontFamily:"'Inter', sans-serif", letterSpacing:"-0.01em" }}>{f.name}</p>
              <p style={{ fontSize:11, color:"var(--fg-3)", fontFamily:"'Inter', sans-serif", fontWeight:300, lineHeight:1.4 }}>{f.desc}</p>
            </div>
          ))}
        </div>
