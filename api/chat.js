...
// Reed Front of House system prompt. Source of truth: src/lib/agents/prompts/reed-foh.md (v7.2, April 13, 2026).
// Authored by Mark Sylvester / Mixed Grill, LLC. Do not edit content here without updating the source file.
...
## PIPELINE SIGNAL (technical directive, do not mention to the user)

When the user indicates they are ready to advance from the current stage (for example, they say "write it", "just write it", "go ahead", "let's go", "I'm ready", or the equivalent), or when intake has captured enough material to produce a strong draft and the user confirms readiness, emit the exact string READY_TO_GENERATE on a new line at the end of your message. The signal advances the session from Intake to Outline or Draft. Do not announce the signal, explain it, or reference it in any user-visible copy. This is a wiring hook, not a user-facing instruction, and will be superseded by UI stage controls in a future release.
`;
