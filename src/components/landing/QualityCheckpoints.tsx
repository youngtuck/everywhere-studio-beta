  ...
  return (
    <section style={{ padding:"100px 32px", background:"var(--bg-2)", borderTop:"1px solid var(--line)" }}>
      <div style={{ maxWidth:760, margin:"0 auto" }}>
        ...
        <h2 style={{ fontFamily:"'Inter', sans-serif", fontSize:"clamp(32px,3.8vw,56px)", fontWeight:400, letterSpacing:"-0.5px", color:"var(--fg)", marginBottom:16, lineHeight:1.08 }}>
          Blocking checkpoints.<br /><em style={{fontStyle:"normal"}}>Everything clears before ship.</em>
        </h2>
        <p style={{ fontSize:15, color:"var(--fg-3)", lineHeight:1.78, marginBottom:52, fontFamily:"'Inter', sans-serif", fontWeight:300 }}>
          ...
          {CHECKPOINTS.map((g,i) => (
            <div key={i}>
              <button onClick={() => setOpen(open===i?null:i)}
                style={{ display:"flex", alignItems:"center", gap:16, width:"100%", padding:"15px 18px", background:"var(--surface)", border:"1px solid var(--line)", borderRadius:open===i?"8px 8px 0 0":8, cursor:"pointer", textAlign:"left", transition:"background 0.12s", fontFamily:"'Inter', sans-serif" }}
                ...
                <span style={{ fontSize:13, color:"var(--fg-3)", fontFamily:"'Inter', sans-serif", fontStyle:"normal", minWidth:12, fontWeight:400 }}>{"\u2022"}</span>
              ...
              {open===i && (
                <div style={{ padding:"14px 18px 18px 44px", background:"var(--bg-3)", border:"1px solid var(--line)", borderTop:"none", borderRadius:"0 0 8px 8px" }}>
                  <p style={{ fontSize:13, color:"var(--fg-2)", lineHeight:1.72, fontFamily:"'Inter', sans-serif", fontWeight:300 }}>{g.desc}</p>
                </div>
              )}
            </div>
          ))}
