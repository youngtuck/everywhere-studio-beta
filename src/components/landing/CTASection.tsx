      ...
      <div style={{ maxWidth:560, margin:"0 auto" }}>
        ...
        {!done ? (
          <form onSubmit={async e=>{e.preventDefault(); await supabase.from("waitlist").insert({ email: email.trim() }).then(() => {}, () => {}); setDone(true);}}
            ...
            <button type="submit"
              style={{ background:"#fff", color:"#000", border:"none", cursor:"pointer", fontSize:13, fontWeight:600, padding:"12px 22px", borderRadius:7, fontFamily:"'Afacad Flux', sans-serif", transition:"opacity 0.15s", letterSpacing:"-0.01em" }}
          ...
          <div style={{ padding:"14px 24px", background:"rgba(74,144,217,0.12)", border:"1px solid rgba(74,144,217,0.24)", borderRadius:8, display:"inline-block", marginBottom:20 }}>
            <p style={{ color:"#4A90D9", fontSize:14, fontFamily:"'Afacad Flux', sans-serif" }}>You're on the list. We'll be in touch.</p>
          </div>
        )}
        <p style={{ fontSize:12, color:"rgba(255,255,255,0.18)", fontFamily:"'Afacad Flux', sans-serif" }}>
          Or <button onClick={()=>navigate("/studio/dashboard")} style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(255,255,255,0.32)", fontSize:12, fontFamily:"'Afacad Flux', sans-serif", textDecoration:"underline" }}>explore the demo</button>
        </p>
      </div>
    </section>
  );
